Shader "UI/SDF_Icon_RadialFill"
{
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            static const float2 domainFreeUV[4]= { float2(0, 0), float2(0, 1), float2(1, 1), float2(1, 0) };

            inline float2 CartesianToPolar(float2 cartCoords)
            {
                float2 polarCoords;
                polarCoords.x = length(cartCoords); //r
                polarCoords.y = atan2(cartCoords.y, cartCoords.x) * (1.0f / 6.28318f); //theta
                return polarCoords;
            }
            ...
            v2f vert(appdata_t v, uint id : SV_VertexID)
            {
                ...
                OUT.domainFreeUV = domainFreeUV[id];
                OUT.color = v.color;
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float t = frac(CartesianToPolar(IN.domainFreeUV - 0.5f).y - 0.25f) + _FillAmount > 1 ? 1 : 0;
                faceColor = lerp(faceColor, _Color.rgb, t);
                color.rgb = lerp(faceColor, outlineColor, outlineAlpha);
                color *= alpha;
                
                float shadowDistance = tex2D(_MainTex, IN.texcoord.xy + shadowOffset).a * 2;

                color *= lerp(IN.color.a, _Color.a, t);
